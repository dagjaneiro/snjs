...
import { StandardFile, SFModelManager } from 'standard-file-js';
...
export class SNComponentManager {
  ...
  */
  ...
  async handleSaveItemsMessage(component, message) {
    ...
    this.runWithPermissions(component, requiredPermissions, async () => {
      ...
      */
      ...
      const localItems = await this.modelManager.mapResponseItemsToLocalModels(
        responseItems,
    ...
    params.callback = approved => {
      if (approved) {
        for (const permission of permissions) {
          const matchingPermission = component.permissions.find(
  ...
  focusChangedForComponent(component) {
    const focused =
      document.activeElement === this.iframeForComponent(component);
    for (var handler of this.handlers) {
