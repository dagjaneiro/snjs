...
export class SNComponentManager {
  ...
  static ClientDataDomain = 'org.standardnotes.sn.components';
  ...
  */
    ...
    this.modelManager.addItemSyncObserver(
      ...
      (allItems, validItems, deletedItems, source, sourceKey) => {
        let syncedComponents = allItems.filter(item => {
          return (
            item.content_type === 'SN|Component' ||
      ...
      */
        ...
        for (var component of syncedComponents) {
          var activeComponent = find(this.activeComponents, {
            uuid: component.uuid
          });
        ...
        for (let observer of this.contextStreamObservers) {
          ...
          for (let handler of this.handlers) {
            ...
            if (handler.contextRequestHandler) {
              var itemInContext = handler.contextRequestHandler(
                observer.component
              );
              if (itemInContext) {
                var matchingItem = find(allItems, {
  ...
  configureForNonMobileUsage() {
    ...
    window.addEventListener
      ...
      : (window as any).attachEvent('onfocusout', detectFocusChange);
    window.addEventListener
      ? window.addEventListener('blur', detectFocusChange, true)
      : (window as any).attachEvent('onblur', detectFocusChange);
  ...
  setComponentHidden(component, hidden) {
    ...
    */
    if (hidden) {
      ...
      // streamContextItem
      let contextObserver = find(this.contextStreamObservers, {
      ...
      // streamItems
      let streamObserver = find(this.streamObservers, {
        identifier: component.uuid
      });
      if (streamObserver) {
  ...
  jsonForItem(item: SFItem, component, source?): SFItemData {
    var params: any = {
      uuid: item.uuid,
  ...
  sendItemsInReply(component, items, message, source?) {
    if (this.loggingEnabled) {
      console.log(
        'Web|componentManager|sendItemsInReply',
        component,
        items,
        message
      );
  ...
  sendContextItemInReply(component, item, originalMessage, source?) {
    if (this.loggingEnabled) {
      console.log(
        'Web|componentManager|sendContextItemInReply',
        component,
        item,
        originalMessage
      );
  ...
  sendMessageToComponent(component, message) {
    ...
    if (
      ...
      if (this.loggingEnabled) {
        console.log(
          'Component disabled for current item, not sending any messages.',
          component.name
        );
      }
      return;
    ...
    if (component.offlineOnly || (this.isDesktop && component.local_url)) {
      return (
        ...
        component.local_url.replace(
          'sn://',
          offlinePrefix + this.desktopManager.getApplicationDataPath() + '/'
        )
      );
    } else {
      let url = component.hosted_url || component.legacy_url;
      if (this.isMobile) {
        let localReplacement =
          this.platform == 'ios' ? 'localhost' : '10.0.2.2';
        url = url
  ...
  handleMessage(component, message) {
    if (!component) {
      ...
      this.alertManager.alert({
        text:
          'An extension is trying to communicate with Standard Notes, but there is an error establishing a bridge. Please restart the app and try again.'
      });
      return;
    }
  ...
  handleStreamItemsMessage(component, message) {
    ...
    this.runWithPermissions(component, requiredPermissions, () => {
      ...
      for (var contentType of message.data.content_types) {
        items = items.concat(
          this.modelManager.validItemsForContentType(contentType)
        );
      }
  ...
  isItemIdWithinComponentContextJurisdiction(uuid, component) {
    let itemIdsInJurisdiction = this.itemIdsInContextJurisdictionForComponent(
      component
    );
  ...
  async handleSaveItemsMessage(component, message) {
    ...
    let itemIdsInContextJurisdiction = this.itemIdsInContextJurisdictionForComponent(
      component
    );
    ...
    if (pendingResponseItems.length > 0) {
      let requiredContentTypes = uniq(
        pendingResponseItems.map(i => {
    ...
    this.runWithPermissions(component, requiredPermissions, async () => {
      ...
      */
      ...
      let localItems = await this.modelManager.mapResponseItemsToLocalModels(
        responseItems,
  ...
  handleDeleteItemsMessage(component, message) {
    var requiredContentTypes = uniq(
      message.data.items.map(i => {
        return i.content_type;
      })
    ).sort();
    var requiredPermissions = [
  ...
  runWithPermissions(component, requiredPermissions, runFunction) {
    ...
    for (let required of requiredPermissions.slice()) {
      ...
      let respectiveAcquired = acquiredPermissions.find(
  ...
  promptForPermissions(component, permissions, callback) {
    ...
    params.permissionsString = this.permissionsStringForPermissions(
      permissions,
      component
    );
    ...
    params.callback = approved => {
      if (approved) {
        for (let permission of permissions) {
          let matchingPermission = component.permissions.find(
      ...
      this.permissionDialogs = this.permissionDialogs.filter(pendingDialog => {
        ...
        const containsObjectSubset = function(source, target) {
          return !target.some(
            val =>
              !source.find(
                candidate => JSON.stringify(candidate) === JSON.stringify(val)
              )
          );
        };
  ...
  async registerComponentWindow(component, componentWindow) {
    ...
    component.sessionKey = await this.SFJS.crypto.generateUUID();
